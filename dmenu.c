...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
      ...
      if (!fullwidth)
		...
		/* these options take one argument */
			...
			colortemp[7] = argv[++i];
		else if (!strcmp(argv[i], "-mb"))  /* mid background color */
			colortemp[8] = argv[++i];
		else if (!strcmp(argv[i], "-mf"))  /* mid foreground color */
			colortemp[9] = argv[++i];
  ...
  if ( colortemp[7])
	...
	if ( colortemp[8])
	   colors[SchemeMid][ColBg]  = strdup(colortemp[8]);
	if ( colortemp[9])
	   colors[SchemeMid][ColFg]  = strdup(colortemp[9]);
